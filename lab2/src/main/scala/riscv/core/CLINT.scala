...
class CLINT extends Module {
  ...
  //lab2(CLINTCSR)
  /*
  val csr_interrupt_enable = io.csr_bundle.mstatus(3)

  when(io.interrupt_flag =/= InterruptStatus.None && csr_interrupt_enable) {
    io.csr_bundle.mstatus_write_data := io.csr_bundle.mstatus | 0x8.U
    io.csr_bundle.mepc_write_data := instruction_address
    io.csr_bundle.mcause_write_data := io.interrupt_flag
    ...
    io.csr_bundle.mstatus_write_data := io.csr_bundle.mstatus & 0xF7.U
